...
cp ${HOME}/fabric/org1/peer1/assets/tls-ca/tls-ca-cert.pem org1/msp/tlscacerts/tls-ca-cert.pem
```
...
在启动服务前，首先创建docker network来方便容器之间通信`docker network create fabric-ca`
...
services:
  org-peer:
    ...
    volumes:
      ...
      //对应挂载的位置
      - ${HOME}/fabric/org1/peer1:/etc/hyperledger/org1/peer1
    networks:
      - fabric-ca
```
...
其中CORE_PEER_ADDRESS，CORE_PEER_MSPCONFIGPATH，CORE_PEER_TLS_CERT_FILE，CORE_PEER_TLS_KEY_FILE，CORE_PEER_TLS_ROOTCERT_FILE，CORE_PEER_GOSSIP_EXTERNALENDPOINT需要根据实际情况进行配置，对应的位置为
    ...
    ${HOME}/fabric/org1/peer1/${relative_root} =/etc/hyperledger/org1/peer1/${relative_rrot}
